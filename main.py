...
WAKE_JARVIS  = "jarvis"   # → local Ollama  (male, system tasks)
WAKE_FRIDAY  = "friday"   # → Groq cloud AI (female, search/browse/fast answers)

OLLAMA_MODEL = "gemma4:e4b"
OLLAMA_HOST  = "http://localhost:11434"
GROQ_MODEL   = "llama-3.1-8b-instant"

VOICE_JARVIS = "en-GB-RyanNeural"    # British male
VOICE_FRIDAY = "en-US-AriaNeural"    # American female
...
MUSIC_LIBRARY = {
    ...
    "energy" : "https://www.youtube.com/watch?v=27p7NO7Q4yY&list=RD27p7NO7Q4yY&start_radio=1",
}
...
# ──────────────────────────────────────────────
# Which assistant is currently active: "jarvis" or "friday"
...
async def _synthesise(text: str, filepath: str, voice: str) -> None:
...
JARVIS_SYSTEM = (
    ...
    "You specialise in system tasks, technical questions, and local operations. "
...
FRIDAY_SYSTEM = (
    ...
    "You specialise in fast answers, web searches, browsing, and general knowledge. "
    "Keep answers short — 2-3 sentences max. No markdown or special characters."
)
...
def handle_battery(command: str) -> bool:
    ...
    try:
        ...
        if bat is None:
            speak("No battery detected — you may be on a desktop."); return True
        pct     = int(bat.percent)
        ...
        speak(f"Battery at {pct}%, {'plugged in' if plugged else 'on battery'} {time_msg}.".strip())
    except Exception as e:
...
def handle_clipboard(command: str) -> bool:
    if "clipboard" not in command:
        return False
    try:
        if "read" in command or "what" in command or "paste" in command:
    ...
    try:
        ...
        for w in command.split():
            if w.isdigit():
                ...
                speak(f"Volume set to {level}."); return True
    ...
    try:
        ...
        for w in command.split():
            if w.isdigit():
                ...
                speak(f"Brightness set to {lvl}."); return True
        if any(k in command for k in ("up", "increase", "raise")):
            sbc.set_brightness(min(100, cur + 10), display=0)
            speak(f"Brightness increased."); return True
        if any(k in command for k in ("down", "decrease", "lower")):
            sbc.set_brightness(max(0, cur - 10), display=0)
            speak(f"Brightness decreased."); return True
    except Exception as e:
...
def handle_play_music(command: str) -> bool:
    ...
    speak(f"Sorry, I don't have {query} in the library.")
    return True

def handle_news() -> None:
    try:
        url = (f"https://newsapi.org/v2/top-headlines"
               f"?country=in&pageSize={MAX_NEWS}&apiKey={NEWS_API_KEY}")
...
def handle_exit(command: str) -> bool:
    ...
    if any(k in command for k in ("sleep", "pause")):
        speak("Going to sleep. Call my name to wake me.")
        return True
    return False

# ──────────────────────────────────────────────
# COMMAND ROUTERS  (one per assistant)
# ──────────────────────────────────────────────
def process_jarvis(command: str) -> None:
    """Jarvis handles system + local tasks. Falls back to local Ollama."""
...
def process_friday(command: str) -> None:
    """Friday handles search, browse, fast cloud answers."""
    c = command.lower().strip()
...
def _calibrate_once(recognizer):
...
def listen_for_audio(recognizer, timeout=3, phrase_limit=5):
    with sr.Microphone() as source:
        try:
...
def flush_microphone(recognizer, duration=0.2):
    try:
        with sr.Microphone() as source:
            recognizer.listen(source, timeout=duration, phrase_time_limit=duration)
    except Exception:
        pass
...
def audio_loop():
    ...
    _calibrate_once(recognizer)
    speak_jarvis(f"Dual assistant online. Say {WAKE_JARVIS} for local AI, or {WAKE_FRIDAY} for cloud AI.")

    while True:
        try:
            set_state(State.IDLE)
            word = listen_for_audio(recognizer, timeout=3, phrase_limit=3)
            if not word:
                continue
            ...
            # ── Detect which assistant was called ──
            if WAKE_JARVIS in word_lower:
                set_active("jarvis")
                speak_jarvis("Yes Boss?")
            elif WAKE_FRIDAY in word_lower:
                set_active("friday")
            ...
            # time.sleep(0.5)
            flush_microphone(recognizer, duration=0.2)

            set_state(State.LISTENING)
            print("[Listening for command...]")
            command = listen_for_audio(recognizer, timeout=6, phrase_limit=10)
            ...
            if _ui_root:
                _ui_root.after(0, _ui_root.destroy)
            break
...
# ──────────────────────────────────────────────
# UI  — Dual-Assistant Sci-Fi Dashboard
# ──────────────────────────────────────────────
def run_ui():
    global _ui_root
    ...
    C_BG      = "#020b14"
    C_PANEL   = "#040f1c"
    C_JARVIS  = "#00d4ff"   # cyan  — Jarvis
    C_FRIDAY  = "#ff69b4"   # pink  — Friday
    C_DIM     = "#0d2535"
    C_DIM_TXT = "#2a4a5e"
    C_GOLD    = "#f0a500"
    C_GREEN   = "#00ff88"
    ...
    try:
        FNT_TITLE  = tkfont.Font(family="Courier New", size=18, weight="bold")
        ...
        FNT_TITLE  = tkfont.Font(size=18, weight="bold")
    ...
    tk.Label(top, text="J.A.R.V.I.S", font=FNT_TITLE,
             bg=C_BG, fg=C_JARVIS).pack(side=tk.LEFT)
    tk.Label(top, text=" + ", font=FNT_TITLE,
             bg=C_BG, fg=C_DIM_TXT).pack(side=tk.LEFT)
    tk.Label(top, text="F.R.I.D.A.Y", font=FNT_TITLE,
             bg=C_BG, fg=C_FRIDAY).pack(side=tk.LEFT)

    clock_var = tk.StringVar()
    tk.Label(top, textvariable=clock_var, font=FNT_SUB,
             bg=C_BG, fg=C_TEXT).pack(side=tk.RIGHT, pady=(8, 0))

    # Divider
    d = tk.Canvas(root, height=1, bg=C_BG, highlightthickness=0)
    d.pack(fill=tk.X, padx=20)
    d.create_line(0, 0, 900, 0, fill=C_DIM_TXT)
    ...
    tk.Label(left, text=f"⚙  {OLLAMA_MODEL}",
             font=FNT_BADGE, bg=C_BG, fg=C_JARVIS).pack(anchor=tk.W, pady=(2, 0))
    tk.Label(left, text=f"☁  {GROQ_MODEL}",
             font=FNT_BADGE, bg=C_BG, fg=C_FRIDAY).pack(anchor=tk.W, pady=(0, 10))

    # Battery
    tk.Label(left, text="BATTERY", font=FNT_SUB, bg=C_BG, fg=C_DIM_TXT).pack(anchor=tk.W)
    bat_var = tk.StringVar(value="-- %")
    tk.Label(left, textvariable=bat_var, font=FNT_STATUS,
             bg=C_BG, fg=C_TEXT).pack(anchor=tk.W, pady=(2, 0))
    ...
    log_wrap = tk.Frame(right, bg=C_PANEL, highlightthickness=1,
                        highlightbackground=C_DIM_TXT)
    ...
    tk.Label(right, textvariable=cmd_var, font=FNT_LOG,
             bg=C_BG, fg=C_TEXT, wraplength=580, justify=tk.LEFT).pack(anchor=tk.W)
    ...
    for i in range(NUM_BARS):
        x = ox + i * (bar_w + gap)
        bars.append(vis.create_rectangle(x, 65, x + bar_w, 65, fill=C_DIM, outline=""))
    ...
    def update_vis():
        ...
        for i, bid in enumerate(bars):
            ...
            if state == State.LISTENING:
                ...
                h = int((wave + random.uniform(0, 0.4)) * 50) + 8
                ...
                color = C_DIM_TXT
            vis.coords(bid, x, 65 - h, x + bar_w, 65 + h)
            vis.itemconfig(bid, fill=color)

        root.after(55, update_vis)

    # ── State polling ──
    _prev = {"heard": "", "reply": "", "who": ""}

    def poll():
        # Active badge
        asst = active_assistant
        if asst == "friday":
            active_var.set("● FRIDAY"); active_lbl.config(fg=C_FRIDAY)
        else:
            active_var.set("● JARVIS"); active_lbl.config(fg=C_JARVIS)

        # State badge
        s = ui_state
        cfg = {
            State.LISTENING: ("● LISTENING", C_JARVIS if asst == "jarvis" else C_FRIDAY),
        ...
        if last_reply and last_reply != _prev["reply"]:
            ...
            tag = "friday" if who == "friday" else "jarvis"
            prefix = "FRI   ▶" if who == "friday" else "JAR   ▶"
            append_log(f"{prefix}  {last_reply}", tag)
        ...
        try:
            ...
            if b:
                bat_var.set(f"{'⚡' if b.power_plugged else '🔋'} {int(b.percent)}%")
        except:
            pass
